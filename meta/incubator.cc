	...
	: params(v), tpool(params[DEFAULT].max_threads), gmutex(99999), tmutex(99999)
{
	...
	// check & delete if meta file exists
	string meta_file = params[DEFAULT].output_gtf_file;
	string cmd = "rm -f " + meta_file;
	std::system(cmd.c_str());

	// check & delete if indv files exist
	string out_dir = params[DEFAULT].output_gtf_dir;
	string cmd2 = "rm -rf " + out_dir + "/*";
...
int incubator::resolve()
{
	...
	printf("before writing circ trsts size:%lu\n",circ_trsts.size());
	postprocess();
...
{	
	...
	*/
	...
	for(int k = 0; k < v.size(); k++)
	{
		if(v[k].splices.size() >= 1) continue; //single exon assemble
...
int incubator::assemble(bundle_group &g, int rid, int gi)
{
	...
	for(int k = 0; k < g.gvv.size(); k++)
	{
		...
		boost::asio::post(this->tpool, [this, &g, &mtx, k, gv, rid, sid, instance]{ 
				...
				asmb.resolve_circ(gv);

				// protect shared circ_trsts
				{
					std::lock_guard<std::mutex> lock(this->tlock);
					circ_trsts.insert(circ_trsts.end(),
									asmb.circ_trsts.begin(),
									asmb.circ_trsts.end());
				}

				// circ_trsts.insert(circ_trsts.end(),asmb.circ_trsts.begin(),asmb.circ_trsts.end());
...
int incubator::postprocess()
{
	...
	boost::asio::post(pool2, [this] { 
		this->write_combined_circgtf();
		// this->write_combined_gtf(); 
	});
	for(int i = 0; i < samples.size(); i++)
	{
		boost::asio::post(pool2, [this, i]{ 
				...
				this->write_individual_circgtf(i);
...
int incubator::write_combined_circgtf()
{
	for(int i=0;i<circ_trsts.size();i++)
	{
		circular_transcript circ = circ_trsts[i];
		circ.write(meta_gtf);
	}
...
int incubator::write_individual_circgtf(int sid)
{
	sample_profile &sp = samples[sid];
	sp.gtf_lock.lock();
	...
	{
		circular_transcript circ = circ_trsts[i];
		if(circ.sid == -1) circ.sid = samples.size() - 1;
		if(circ.sid != sid) continue;
		circ.write(*sp.individual_gtf);
	}
