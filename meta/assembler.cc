...
int assembler::resolve_circ(vector<bundle*> gv)
{
	...
	for(int k = 0; k < gv.size(); k++)
	{
		...
		// insert candidate partially unbridged circs
		all_unbridged_candidate_trsts.insert(gv[k]->unbridged_candidate_trsts.begin(), gv[k]->unbridged_candidate_trsts.end());
		mylock.unlock();
		printf("assembler circ trsts size:%lu\n", circ_trsts.size());
...
int assembler::resolve(vector<bundle*> gv)
{
	...
	// {
	// 	gv[k]->build_fragments();
	// 	gv[k]->bridge(); //hits cleared in this function?
	// }
...
int assembler::bridge_circ(vector<bundle*> gv)
{
	assert(gv.size() >= 2);
	...
	for(int k = 0; k < gv.size(); k++)
	{
		...
		graph_cluster gc(gr, bd, cfg.max_reads_partition_gap, true); //store hits true tasfia
		...
		printf("assembler circ trsts size after meta assembly:%lu\n",circ_trsts.size());
	}
	cb.clear();
	return 0;
}
