...
int bundle_base::build_fragments()
{
	...
	printf("bundle base hits size:%d\n", hits.size());
...
int bundle_base::count_unbridged_circ()
{
	int unbridged = 0;
	for(int i = 0; i < circ_frgs.size(); i++)
	{
		// only group unbridged fragments
		if(circ_frgs[i][2] >= 1) continue;
		if(circ_frgs[i][2] <= -1) continue;
		unbridged++;
	}
	return unbridged;
...
int bundle_base::update_bridges_circ(const vector<int> &frlist, const vector<int32_t> &chain, int strand)
{
	int cnt = 0;
	for(int i = 0; i < frlist.size(); i++)
	{
		assert(chain.size() % 2 == 0);
		assert(frlist.size() == 1); // only one circRNA in cluster
...
int bundle_base::update_bridges(const vector<int> &frlist, const vector<int32_t> &chain, int strand)
{
	...
	for(int i = 0; i < frlist.size(); i++)
	{
		...
		if(frgs[k][2] != 0)
		{
			printf("frgs type not zero, type is %d, read:%s",frgs[k][2],hits[frgs[k][0]].qname.c_str());
		}
		assert(frgs[k][2] == 0);
