...
int graph_cluster::group_pereads()
{
	...
	{
		if(bd.hits[bd.frgs[i][0]].qname == "E00512:127:HJNF3ALXX:1:2211:3681:60079")
		{
			printf("E00512:127:HJNF3ALXX:1:2211:3681:60079 in regular frgs group pereads, type:%d\n",bd.frgs[i][2]);
		}
...
int graph_cluster::build_pereads_clusters_circ(vector<pereads_cluster> &vc_circ)
{
	for(int i=0;i<bd.circ_frgs.size();i++)
	{
		// only create clusters for unbridged fragments
		if(bd.circ_frgs[i][2] >= 1) continue;
		if(bd.circ_frgs[i][2] <= -1) continue;

		bd.circ_frgs[i][2] = -1;		// assume cannot be bridged

		int h1 = bd.circ_frgs[i][0];
		int h2 = bd.circ_frgs[i][1];
		...
		pc.bounds[3] = bd.hits[h2].rpos;

		// printf("printing circ cluster info:\n");
		// printf("h1pos:%d,h1rpos:%d,h2pos:%d,h2rpos:%d\n",bd.hits[h1].pos,bd.hits[h1].rpos,bd.hits[h2].pos,bd.hits[h2].rpos);
		//pc.extends empty
