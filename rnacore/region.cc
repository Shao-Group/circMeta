...
region::region(int32_t _lpos, int32_t _rpos, int _ltype, int _rtype, const split_interval_map *_mmap, const split_interval_map *_imap, const parameters &c, const sample_profile &s, bundle_base &bundle)
	:lpos(_lpos), rpos(_rpos), mmap(_mmap), imap(_imap), ltype(_ltype), rtype(_rtype), cfg(c), sp(s), bd(bundle)
{
...
int region::build_partial_exons()
{
	...
	for(JIMI it = jmap.begin(); it != jmap.end(); it++)
	{
		...
		if(bd.chrm == "chr2" && p1 >= 164291946 && p1 <= 164304556)
		{
			printf("got jmap entry chr2, p1:%d, p2:%d; region lpos:%d, region rpos:%d\n",p1,p2,lpos,rpos);
		}
		if(bd.chrm == "chr8" && p2 >= 19258624 && p2 <= 19260700)
		{
			printf("got jmap entry chr8, p1:%d, p2:%d; region lpos:%d, region rpos:%d\n",p1,p2,lpos,rpos);
		}
		
		// bool b = empty_subregion(p1, p2); //commented for meta circ assembly
		bool b = false;
		...
		if(bd.chrm == "chr2" && p1 >= 164291946 && p1 <= 164304556)
		{
			printf("filtered jmap entry chr2, p1:%d, p2:%d\n",p1,p2);
		}
		if(bd.chrm == "chr8" && p2 >= 19258624 && p2 <= 19260700)
		{
			printf("filtered jmap entry chr8, p1:%d, p2:%d\n",p1,p2);
		}
