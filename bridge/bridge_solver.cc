	...
	: gr(g), vc(v), cfg(c)
{
	...
	printf("size of v:%d\n",v.size());
	printf("size of vpairs:%d\n",vpairs.size());
	build_piers();
	printf("size of piers:%d\n",piers.size());
	nominate();
	vote();
...
int bridge_solver::build_bridging_vertices()
{
	...
	for(int i = 0; i < vc.size(); i++)
	{
		...
		{
			// pc.print(i+1);
			// printf("bound1:%d and bound2:%d\n",pc.bounds[1],pc.bounds[2]);
		}
...
int bridge_solver::build_piers()
{
	...
	for(int k = 0; k < vc.size(); k++)
	{
		...
		// if(vc[k].is_circ == true)
		// {
		// 	printf("p.first:%d and p.second:%d\n",p.first,p.second);
		// }
...
int bridge_solver::vote(int r, bridge_path &bbp)
{
	...
	if(chains.size() == 0) return 0; //single exon circrna path type -1!
